...
def query_trace_response(
    ...
    candidates: dict[TraceKey | _MalformedDropKey, _CandidateTrace] = {}
    ...
    if vector is not None:
        ...
        for match in vector.chunk_matches:
            ...
            best_vector_scores[match.key] = max(
                best_vector_scores.get(match.key, 0.0),
                match.similarity,
            )
        ...
        for rejection in vector.drops:
            try:
                ...
                entry.scores.vector_similarity = max(
                    entry.scores.vector_similarity or 0.0,
            ...
            if key not in surviving_vector_keys:
                ...
                if current_rejection is None or (
                    ...
                    entry.rejection = rejection
                best_vector_scores[key] = max(
                    best_vector_scores.get(key, 0.0),
                    rejection.similarity,
                )
    ...
    if vector is not None:
        ...
        stages.extend(
            [
                ...
                InspectQueryStage(
                    name="row_collapse",
                    count_in=len(vector.chunk_matches),
    ...
    response_candidates += [
        ...
        for entry in sorted(
            (entry for entry in candidates.values() if entry.key not in returned_final_keys),
            key=lambda item: (
    ...
    return InspectQueryResponse(
        ...
        engine=InspectQueryEngine(
            ...
            ready_rows=readiness.ready_rows if readiness is not None else 0,
            pending_rows=readiness.pending_rows if readiness is not None else 0,
            other_identity_rows=(readiness.other_identity_rows if readiness is not None else 0),
