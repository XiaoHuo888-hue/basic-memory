...
def build_vector_stage(
    *,
    ...
    chunk_matches: Mapping[TraceKey, Sequence[tuple[str, float]]] | None = None,
    ...
    previous_chunk_matches: dict[TraceKey, list[tuple[str, float]]] = {}
    if previous is not None:
        for match in previous.chunk_matches:
            previous_chunk_matches.setdefault(match.key, []).append(
                (match.chunk_key, match.similarity)
            )
    ...
    return VectorStageTrace(
        ...
        chunk_matches=tuple(
            VectorChunkMatch(key=key, chunk_key=chunk_key, similarity=similarity)
            for key, matches in effective_chunk_matches.items()
            for chunk_key, similarity in matches
        ),
