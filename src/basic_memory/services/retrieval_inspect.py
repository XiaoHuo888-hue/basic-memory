...
async def inspect_entity_chunks(
    ...
    # With semantic indexing off — by config, or by the runtime keyword-only fallback —
    # no chunks are expected at all, so an empty manifest is not missing coverage.
    if await repository.semantic_effectively_enabled():
