...
def test_stage_builders_freeze_plain_values_and_exact_fusion_math():
    vector = build_vector_stage(
        ...
        chunk_matches={
            ("entity", 1): [("entity:1:0", 0.8)],
            ("entity", 2): [("entity:2:0", 0.4)],
        },
        embed_ms=1.25,
        vector_query_ms=2.5,
    )
...
def test_query_response_flattens_every_rejection_variant():
    vector = build_vector_stage(
        ...
        chunk_matches={
            ("entity", 1): [("entity:1:0", 0.9)],
            ("entity", 6): [("entity:6:0", 0.4)],
            ("entity", 7): [("entity:7:0", 0.8)],
            ("entity", 8): [("entity:8:0", 0.7)],
            ("entity", 9): [("entity:9:0", 0.65)],
        },
    )
    ...
    response = query_trace_response(trace, {1: "external-1", 2: "external-2", 11: "external-11"})
...
def test_query_response_keeps_row_when_a_ready_chunk_survives_a_dropped_sibling():
    vector = build_vector_stage(
        ...
        chunk_matches={
            ("entity", 1): [("entity:1:0", 0.8), ("entity:1:2", 0.7)],
            ("entity", 2): [("entity:2:0", 0.9)],
        },
    )
...
def test_query_response_clears_vector_rejection_for_fts_fusion_survivor():
    ...
    vector = build_vector_stage(
        ...
        chunk_matches={("entity", 1): [("entity:1:0", 0.1)]},
    )
    fusion = build_fusion_stage(
        formula_version="max+0.3*min/v1",
        bonus=FUSION_BONUS,
    ...
    candidates = {candidate.id: candidate for candidate in query_trace_response(trace).candidates}
...
def test_identically_malformed_keys_from_two_entities_stay_distinct_candidates():
    ...
    vector = build_vector_stage(
        ...
        chunk_matches={("entity", 1): [("entity:1:0", 0.9)]},
    )
    collector = SearchTraceCollector(
        vector=vector,
        readiness=ManifestReadiness("i", "m", 1, 0, 0),
